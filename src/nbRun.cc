...
//		obj.fParticleName = particleName;
//		obj.fcount = 1;
//		particleDataVector.push_back(obj);
//    G4cout<<particleDataVector.size()<<G4endl;
//    
...
//    G4cout<<""<<G4endl;
 //}
 std::map<G4String, ParticleDataPerLayer>::iterator it = fParticlePerLayerDataMap.find(layerName);
  if ( it == fParticlePerLayerDataMap.end()) {
    fParticlePerLayerDataMap[layerName] = ParticleDataPerLayer(1);
  }
  else {
    ParticleDataPerLayer& data = it->second;
    data.fCount++;
  }   
...
void nbRun::SetTimeWindow(G4double t1, G4double t2)
{
  fTimeWindow1 = t1;
  fTimeWindow2 = t2;
}
...
void nbRun::CountInTimeWindow(G4String name, G4bool life1,
...
void nbRun::EventTiming(G4double time)
{
  fTimeCount++;  
  fEventTime[0] += time;
...
void nbRun::PrimaryTiming(G4double ptime)
{
  fPrimaryTime += ptime;
}
...
void nbRun::EvisEvent(G4double Evis)
{
  fEvisEvent[0] += Evis;
...
void nbRun::EndOfRun() 
{
   G4cout<<"this is end of run"<<G4endl;
