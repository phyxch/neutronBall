...
nbTrackingAction::nbTrackingAction(nbEventAction* EA)
:G4UserTrackingAction(),
 fEvent(EA),fTrackMessenger(0),
 fFullChain(true)
 
{
  fTrackMessenger = new nbTrackingMessenger(this);   
...
nbTrackingAction::~nbTrackingAction()
{
...
void nbTrackingAction::SetTimeWindow(G4double t1, G4double dt)
{
  fTimeWindow1 = t1;
...
void nbTrackingAction::PostUserTrackingAction(const G4Track* track)
{
  //keep only ions
  //
  if (fCharge < 3. ) return;
