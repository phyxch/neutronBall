...
#include "nbTrackingMessenger.hh"
...
nbTrackingMessenger::nbTrackingMessenger(nbTrackingAction* trackA)
:G4UImessenger(),
 fTrackingAction(trackA),fTrackingCmd(0),fTimeWindowCmd(0)
{
  fTrackingCmd = new G4UIcmdWithABool("/neutronBall/fullChain",this);
  fTrackingCmd->SetGuidance("allow full decay chain");
  fTrackingCmd->SetParameterName("flag",true);
  fTrackingCmd->SetDefaultValue(true);

 fTimeWindowCmd = new G4UIcommand("/neutronBall/timeWindow",this);
...
nbTrackingMessenger::~nbTrackingMessenger()
{
  delete fTrackingCmd;
...
void nbTrackingMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{ 
